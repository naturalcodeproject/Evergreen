...
final class Config
{
	...
	{
		$this->physical_path = dirname(dirname(__FILE__));
	...
	public function set_uri()
	{
		switch ($this->uri_type)
		{
			...
			default:
				...
				$this->working_uri = $_SERVER['REQUEST_URI'];
			break;
		}
	}
	...
	public function check_uri()
	{
		...
		foreach($uri_to_use as $key => $item)
		{
			...
			if ($count == 0 && !file_exists($this->physical_path."/branches/".$branch_name."/controllers/".$url_vals[$count].".php") && !empty($branch_name))
			{
				$url_vals = array_merge(array($item), $url_vals);
			}
			elseif ($count == 0 && !file_exists($this->physical_path."/controllers/".$url_vals[$count].".php") && empty($branch_name))
			{
				$url_vals = array_merge(array($item), $url_vals);
			}
	...
	{
		$routes_to_use = $this->routes;
		
		$uri_to_use = $this->get_uri_map();
		
		foreach($routes_to_use as $regex=>$destination)
		{
			$regex = str_replace("/", "\/", $regex);
			if (preg_match("/^{$regex}/i", $request_uri))
			{
				$new_uri = preg_replace("/{$regex}/i", "{$destination}", $this->get_working_uri());
				$this->current_route = array($regex=>$destination);
				$_SERVER['REQUEST_URI'] = $new_uri;
				$this->working_uri = $new_uri;
				$this->check_uri();
				return true;
			}
		}
